...
class ContactService extends GetxController {
  ...
  var contactDetails = ContactModel(nrc: '', fullNames: '', phoneNumber: '', email: '', roleId: '', createdAt: '', updatedAt: '', profilePicture: '').obs;
...
Future<void> getContact(String nrc) async {
  ...
  if (response.statusCode == 200) {
    ...
    if (responseData['contact'] != null) {
      ...
      Get.offAll(() => NewContact(contactNrc: contactDetails.value.nrc, fullNames: contactDetails.value.fullNames,email: contactDetails.value.email,phoneNumber: contactDetails.value.phoneNumber,profilePicture: contactDetails.value.profilePicture,));
    } else {
...
Future<void> saveContact(Map<String, dynamic> contactBody) async
{
  ...
  if(response.statusCode == 201)
  {
   ...
   contactDetails.value = ContactModel(nrc: '', fullNames: '', phoneNumber: '', email: '', roleId: '', createdAt: '', updatedAt: '', profilePicture: '').obs as ContactModel;;
  }
  else{
    isLoading.value = false;
